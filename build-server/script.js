...
function executeBuild(outDirPath) {
  return new Promise((resolve, reject) => {
    console.log(
      `\n========== Starting build process in directory: ${outDirPath} ==========\n`
    );

    // First, ensure the directory exists and we're in it
    if (!fs.existsSync(outDirPath)) {
      console.error(`❌ Error: Output directory ${outDirPath} does not exist`);
    ...
    installProcess.stdout.on("data", (data) => {
      console.log(`[npm install] 📝 stdout: ${data.trim()}`);
      buildOutput += data;
    });

    installProcess.stderr.on("data", (data) => {
      console.error(`[npm install] ⚠️ stderr: ${data.trim()}`);
      errorOutput += data;
    });
    ...
    installProcess.on("close", (code) => {
      ...
      buildProcess.stdout.on("data", (data) => {
        console.log(`[npm run build] 📝 stdout: ${data.trim()}`);
        buildOutput += data;
      });

      buildProcess.stderr.on("data", (data) => {
        console.error(`[npm run build] ⚠️ stderr: ${data.trim()}`);
        errorOutput += data;
      });
